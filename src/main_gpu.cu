...
__global__ void render_kernel_optimized(float3 *framebuffer,
                                        ...
                                        int max_bounces, bool antialias) {
  ...
  // BEGIN AI EDIT: Antialiasing support
  int samples = antialias ? 4 : 1;
  float3 pixel_color = make_float3(0, 0, 0);

  for (int s = 0; s < samples; s++) {
    float offset_x = float(s % 2) * 0.5f;
    float offset_y = (s >= 2 ? 0.5f : 0.0f);
    float u = (float(x) + offset_x) / float(width - 1);
    float v = (float(y) + offset_y) / float(height - 1);
    GPURay ray = camera.get_ray(u, v);
    // END AI EDIT
    ...
    // BEGIN AI EDIT: Accumulate sample color for antialiasing
    pixel_color = float3_ops::add(pixel_color, final_color);
  } // end antialias samples loop
  // END AI EDIT

  // 4. Store final color in framebuffer
  // BEGIN AI EDIT: Average samples
  framebuffer[pixel_idx] = float3_ops::mul(pixel_color, 1.0f / float(samples));
  // END AI EDIT
}
...
int main(int argc, char *argv[]) {
  ...
  // BEGIN AI EDIT: Parse command-line arguments for scene file and antialias
  // flag
